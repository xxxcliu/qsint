...
/**
    ...
    \image html ScrollPanner.png An example of ScrollPanner

    ScrollPanner is intended for scrolling in both direction (horizontal and vertical)
    ...
    To use ScrollPanner, attach it to the source widget (which content intended to
    be scrolled) using one of connectSource() methods.
    Currently \a QAbstractScrollArea and \a QAbstractSlider descedants can be handled by ScrollPanner.
    ...
    // create ScrollPanner and connect it to the QGraphicsView
    QSint::ScrollPanner *myPanner = new ScrollPanner(this);
    myPanner->connectSource(myView);
    \endcode
*/
class ScrollPanner : public QWidget
{
    Q_OBJECT
...
public:
    /** Constructor */
    explicit ScrollPanner(QWidget *parent = 0);
    /** Destructor */
    virtual ~ScrollPanner();
